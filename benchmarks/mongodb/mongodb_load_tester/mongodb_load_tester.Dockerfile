...
WORKDIR ycsb-0.17.0
RUN sed -i -e 's/recordcount=1000/recordcount=100000/g' ./workloads/workloada && \
    sed -i -e 's/operationcount=1000/operationcount=100000/g' ./workloads/workloada && \
...
ENTRYPOINT ./bin/ycsb load mongodb -s -P workloads/workloada -threads $(grep -c ^processor /proc/cpuinfo) -p mongodb.url=mongodb://root:pass@192.168.208.152:27017
