...
The development of SecurityPerf was motivated by the need to accuratley determine
the overhead of a security solution when it is deployed on diverse production
workloads in a manner that is reproducable by anyone and provides meaningful results
...
realistic benchmarks for common production workloads as long as they have the ability
...
The clientr VM runs the benchmarking software.
...
for all services are tested, a summary file is generated that agregates
the results and calcualted the mean and standard deviation of the results
for each service.
...
One subdirectory for the benchmarking docker image, and one subdirctory for
the service's docker image. Each service contains a python script that defines
a subclass for the benchmark. The parent class is located in `benchmarks/benchmark.py`.
...
distinction is the fact that WordPress actually starts two services rather than one,
Apache and MySQL.

For more detail, take a look at the apache.py script. It is commented well with
